import os, json, warnings
import numpy as np
import pandas as pd
import optuna
from xgboost import XGBClassifier
from sklearn.model_selection import TimeSeriesSplit
from sklearn.metrics import accuracy_score, classification_report
from sklearn.utils.class_weight import compute_sample_weight
from sklearn.calibration import CalibratedClassifierCV
import joblib
warnings.filterwarnings("ignore")

from features import build_features, add_htf_context, load_htf_csv

DATA_DIR      = "data"
MODEL_PATH    = "models/xgb_model.json"
CALIB_PATH    = "models/calibrated_model.pkl"
FEATURES_PATH = "models/feature_cols.txt"
N_TRIALS      = 50
N_SPLITS      = 3

# 1-minute scalping — asymmetric RR
# TP must be 2x SL so even 40% win rate is profitable
LABEL_FORWARD  = 10     # 10 bars = 10 minutes
LABEL_TP_MULT  = 0.5    # TP = 0.5x ATR
LABEL_SL_MULT  = 0.25   # SL = 0.25x ATR — 2:1 RR
CONF_THRESHOLD = 0.55

FEATURE_COLS = [
    "trend_8_21","trend_21_50","trend_50_200",
    "price_vs_ema21","price_vs_ema50","price_vs_ema200",
    "ema21_slope","ema50_slope",
    "rsi14","rsi7","rsi_slope",
    "macd_line","macd_signal","macd_hist","macd_cross","macd_hist_slope",
    "roc5","roc10","roc20",
    "stoch_k","stoch_d","stoch_cross",
    "atr14","atr7","atr_pct","atr_ratio","atr_trend",
    "bb_width","bb_position","bb_squeeze",
    "candle_body","candle_range","upper_wick","lower_wick","is_bullish",
    "consec_bull","consec_bear",
    "rel_volume","volume_spike","price_vs_vwap",
    "above_prev_high","below_prev_low","break_high","break_low",
    "dist_to_high20","dist_to_low20",
    "session_london","session_newyork","session_overlap","session_asian",
    "london_open_spike","ny_open_spike",
    "news_blackout","tradeable",
    "hour","day_of_week","is_monday","is_friday",
    "hour_sin","hour_cos","dow_sin","dow_cos",
    "atr_above_avg",
    "regime_trending","regime_ranging","regime_highvol","regime_lowvol","adx_val",
    "h1_trend_8_21","h1_trend_21_50","h1_trend_50_200","h1_ema21_slope",
    "h1_rsi14","h1_rsi_slope","h1_macd_hist","h1_macd_hist_slope",
    "h1_bb_position","h1_bb_squeeze","h1_atr14","h1_atr_trend",
    "h1_rel_volume","h1_adx_val","h1_regime_trending","h1_regime_ranging",
    "h4_trend_8_21","h4_trend_21_50","h4_trend_50_200","h4_ema21_slope",
    "h4_rsi14","h4_rsi_slope","h4_macd_hist","h4_macd_hist_slope",
    "h4_bb_position","h4_bb_squeeze","h4_atr14","h4_atr_trend",
    "h4_rel_volume","h4_adx_val","h4_regime_trending","h4_regime_ranging",
]


def read_mt5_csv(path):
    df = pd.read_csv(path, sep="\t", header=None,
                     names=["time","open","high","low","close","volume"])
    df["time"] = pd.to_datetime(df["time"], utc=True)
    df = df.set_index("time").sort_index()
    return df.apply(pd.to_numeric, errors="coerce").dropna()


def load_all_data():
    frames = []
    for fname in ["XAUUSD1.csv","XAUUSD5.csv","XAUUSD15.csv"]:
        path = os.path.join(DATA_DIR, fname)
        if not os.path.exists(path):
            continue
        try:
            df = read_mt5_csv(path)
            frames.append(df[["open","high","low","close","volume"]])
            print(f"  Loaded {fname}: {len(df)} rows")
        except Exception as e:
            print(f"  Skipped {fname}: {e}")
    combined = pd.concat(frames).sort_index()
    combined = combined[~combined.index.duplicated(keep="last")]
    print(f"Total rows: {len(combined)}")
    return combined


def make_labels(df,
                forward_bars=LABEL_FORWARD,
                tp_mult=LABEL_TP_MULT,
                sl_mult=LABEL_SL_MULT):
    """
    Asymmetric 2:1 RR labels for 1-minute scalping.
    TP = 0.5x ATR, SL = 0.25x ATR.
    Label BUY only when: TP hit before SL AND price is above EMA21.
    Label SELL only when: TP hit before SL AND price is below EMA21.
    This prevents labelling both directions on same bar.
    """
    atr   = df["atr14"].values
    close = df["close"].values
    high  = df["high"].values
    low   = df["low"].values
    n     = len(df)
    labels = np.ones(n, dtype=int)  # FLAT=1

    for i in range(n - forward_bars):
        if np.isnan(atr[i]) or atr[i] <= 0:
            continue

        entry    = close[i]
        tp_long  = entry + atr[i] * tp_mult
        sl_long  = entry - atr[i] * sl_mult
        tp_short = entry - atr[i] * tp_mult
        sl_short = entry + atr[i] * sl_mult

        long_tp_bar  = None
        long_sl_bar  = None
        short_tp_bar = None
        short_sl_bar = None

        for j in range(i + 1, i + 1 + forward_bars):
            if long_tp_bar is None and high[j] >= tp_long:
                long_tp_bar = j
            if long_sl_bar is None and low[j] <= sl_long:
                long_sl_bar = j
            if short_tp_bar is None and low[j] <= tp_short:
                short_tp_bar = j
            if short_sl_bar is None and high[j] >= sl_short:
                short_sl_bar = j

        long_wins  = (long_tp_bar is not None and
                      (long_sl_bar is None or long_tp_bar < long_sl_bar))
        short_wins = (short_tp_bar is not None and
                      (short_sl_bar is None or short_tp_bar < short_sl_bar))

        if long_wins and not short_wins:
            labels[i] = 2
        elif short_wins and not long_wins:
            labels[i] = 0

    return pd.Series(labels, index=df.index)


def objective(trial, X, y):
    params = {
        "n_estimators":     trial.suggest_int("n_estimators", 100, 800),
        "max_depth":        trial.suggest_int("max_depth", 3, 7),
        "learning_rate":    trial.suggest_float("learning_rate", 0.01, 0.15, log=True),
        "subsample":        trial.suggest_float("subsample", 0.6, 0.95),
        "colsample_bytree": trial.suggest_float("colsample_bytree", 0.5, 0.95),
        "min_child_weight": trial.suggest_int("min_child_weight", 20, 100),
        "gamma":            trial.suggest_float("gamma", 0.0, 3.0),
        "reg_alpha":        trial.suggest_float("reg_alpha", 0.0, 5.0),
        "reg_lambda":       trial.suggest_float("reg_lambda", 0.5, 5.0),
        "tree_method":      "hist",
        "device":           "cpu",
        "eval_metric":      "mlogloss",
        "random_state":     42,
        "n_jobs":           -1,
    }
    conf_thresh = trial.suggest_float("conf_threshold", 0.50, 0.75)

    rng = np.random.default_rng(trial.number)
    idx = np.sort(rng.choice(len(X), size=min(40000, len(X)), replace=False))
    Xs, ys = X[idx], y[idx]

    tscv   = TimeSeriesSplit(n_splits=N_SPLITS)
    scores = []

    for tr, val in tscv.split(Xs):
        weights = compute_sample_weight("balanced", ys[tr])
        m = XGBClassifier(**params)
        m.fit(Xs[tr], ys[tr], sample_weight=weights,
              eval_set=[(Xs[val], ys[val])], verbose=False)

        proba    = m.predict_proba(Xs[val])
        max_conf = proba.max(axis=1)
        pred     = m.classes_[proba.argmax(axis=1)]
        pred_f   = np.where(max_conf >= conf_thresh, pred, 1)

        mask = pred_f != 1
        if mask.sum() < 20:
            scores.append(0.0)
            continue

        win_rate = accuracy_score(ys[val][mask], pred_f[mask])
        coverage = mask.mean()

        if coverage < 0.02:
            win_rate *= 0.3
        if coverage > 0.40:
            win_rate *= 0.5  # penalise overtrade

        n_buy  = (pred_f[mask] == 2).sum()
        n_sell = (pred_f[mask] == 0).sum()
        if n_buy == 0 or n_sell == 0:
            win_rate *= 0.3
        elif min(n_buy, n_sell) / max(n_buy, n_sell) < 0.15:
            win_rate *= 0.7

        scores.append(win_rate)

    return float(np.mean(scores))


def main():
    os.makedirs("models", exist_ok=True)
    print("\n=== GoldBot Pro — M1 Scalper Training ===")
    print(f"Labels: TP={LABEL_TP_MULT}x ATR | SL={LABEL_SL_MULT}x ATR | "
          f"RR=2:1 | forward={LABEL_FORWARD} bars\n")

    print("Loading data...")
    raw = load_all_data()

    print("Building features...")
    df = build_features(raw)

    for prefix, fname in [("h1","XAUUSD60.csv"), ("h4","XAUUSD240.csv")]:
        path = os.path.join(DATA_DIR, fname)
        if os.path.exists(path):
            try:
                df_htf = load_htf_csv(path)
                df     = add_htf_context(df, df_htf, prefix)
                print(f"  HTF {prefix} added ({len(df_htf)} bars)")
            except Exception as e:
                print(f"  HTF {prefix} skipped: {e}")

    print("Generating labels...")
    df["label"] = make_labels(df)

    available_cols = [c for c in FEATURE_COLS if c in df.columns]
    df = df.dropna(subset=["label"] + available_cols)

    X = df[available_cols].values
    y = df["label"].astype(int).values

    buy_n  = (y == 2).sum()
    flat_n = (y == 1).sum()
    sell_n = (y == 0).sum()
    total  = len(y)

    print(f"Dataset: {total} rows | BUY={buy_n} FLAT={flat_n} SELL={sell_n}")
    print(f"Class balance: BUY={buy_n/total:.1%} FLAT={flat_n/total:.1%} "
          f"SELL={sell_n/total:.1%}")

    if buy_n/total < 0.08 or sell_n/total < 0.08:
        print("WARNING: Signal rate below 8% — labels may be too strict")

    print(f"\nRunning Optuna ({N_TRIALS} trials)...")
    optuna.logging.set_verbosity(optuna.logging.WARNING)
    study = optuna.create_study(
        direction="maximize",
        sampler=optuna.samplers.TPESampler(seed=42),
        pruner=optuna.pruners.MedianPruner(n_warmup_steps=10),
    )
    study.optimize(
        lambda t: objective(t, X, y),
        n_trials=N_TRIALS,
        show_progress_bar=True,
    )

    best_params = dict(study.best_params)
    conf_threshold = best_params.pop("conf_threshold", CONF_THRESHOLD)
    best_params.update({
        "tree_method":  "hist",
        "device":       "cpu",
        "eval_metric":  "mlogloss",
        "random_state": 42,
        "n_jobs":       -1,
    })

    print(f"\nBest win-rate proxy : {study.best_value:.4f}")
    print(f"Best conf threshold : {conf_threshold:.3f}")

    print("\nTraining final model...")
    weights = compute_sample_weight("balanced", y)

    final = XGBClassifier(**best_params)
    final.fit(X, y, sample_weight=weights, verbose=False)

    importance    = final.feature_importances_
    selected_cols = [f for f, imp in zip(available_cols, importance)
                     if imp >= 0.003]
    if len(selected_cols) < 20:
        ranked = sorted(zip(available_cols, importance), key=lambda x: -x[1])
        selected_cols = [f for f, _ in ranked[:20]]
    print(f"Feature selection: {len(available_cols)} → {len(selected_cols)} features")

    X_sel  = df[selected_cols].values
    final2 = XGBClassifier(**best_params)
    final2.fit(X_sel, y, sample_weight=weights, verbose=False)

    print("Calibrating probabilities...")
    tscv = TimeSeriesSplit(n_splits=N_SPLITS)
    calibrated = CalibratedClassifierCV(
        XGBClassifier(**best_params), method="isotonic", cv=tscv
    )
    calibrated.fit(X_sel, y)
    joblib.dump(calibrated, CALIB_PATH)

    print("\nEvaluating...")
    cv_scores, wr_scores, cov_scores = [], [], []
    for tr, val in tscv.split(X_sel):
        m = XGBClassifier(**best_params)
        m.fit(X_sel[tr], y[tr],
              sample_weight=compute_sample_weight("balanced", y[tr]),
              verbose=False)
        proba    = m.predict_proba(X_sel[val])
        max_conf = proba.max(axis=1)
        pred     = np.where(max_conf >= conf_threshold,
                            m.classes_[proba.argmax(axis=1)], 1)
        cv_scores.append(accuracy_score(y[val], pred))
        mask = pred != 1
        if mask.sum() > 0:
            wr_scores.append(accuracy_score(y[val][mask], pred[mask]))
            cov_scores.append(mask.mean())

    print(f"Overall CV accuracy  : {np.mean(cv_scores):.4f}")
    print(f"Win rate (non-FLAT)  : {np.mean(wr_scores):.4f}  ← target 0.55+")
    print(f"Signal coverage      : {np.mean(cov_scores):.2%}")
    print(f"Confidence threshold : {conf_threshold:.3f}")
    print(f"Expected RR          : 2:1 (TP={LABEL_TP_MULT}x / SL={LABEL_SL_MULT}x ATR)")

    tscv_list   = list(tscv.split(X_sel))
    tr_l, val_l = tscv_list[-1]
    m_last = XGBClassifier(**best_params)
    m_last.fit(X_sel[tr_l], y[tr_l],
               sample_weight=compute_sample_weight("balanced", y[tr_l]),
               verbose=False)
    pred_l = np.where(
        m_last.predict_proba(X_sel[val_l]).max(axis=1) >= conf_threshold,
        m_last.classes_[m_last.predict_proba(X_sel[val_l]).argmax(axis=1)], 1)

    print("\nClassification Report (last fold):")
    print(classification_report(y[val_l], pred_l,
                                target_names=["SELL","FLAT","BUY"],
                                zero_division=0))

    final2.save_model(MODEL_PATH)
    final2.save_model(MODEL_PATH.replace(".json",".ubj"))
    with open(FEATURES_PATH, "w") as f:
        f.write("\n".join(selected_cols))
    with open("models/best_params.json", "w") as f:
        json.dump({
            "params":          best_params,
            "conf_threshold":  conf_threshold,
            "win_rate":        float(np.mean(wr_scores)),
            "accuracy":        float(np.mean(cv_scores)),
            "coverage":        float(np.mean(cov_scores)),
            "features":        len(selected_cols),
            "label_tp_mult":   LABEL_TP_MULT,
            "label_sl_mult":   LABEL_SL_MULT,
            "label_forward":   LABEL_FORWARD,
            "rr_ratio":        LABEL_TP_MULT / LABEL_SL_MULT,
        }, f, indent=2)

    print(f"\nModel saved    → {MODEL_PATH}")
    print(f"Features saved → {FEATURES_PATH}")
    print(f"Feature count  : {len(selected_cols)}")
    print("=== Training complete ===")


if __name__ == "__main__":
    main()
